...
public:
	MsgForVideo(char* msgData, int iDataen);
	~MsgForVideo();
...
private:
	void splitBytes(char* in, int insize, std::string& dst);

public:
	std::string strPath;
	std::string strThumbFile;

private:
	CodedInputStreamMicro* codeInput;
	std::map<int, MsgForVideo::FieldType> fieldMap;
};
