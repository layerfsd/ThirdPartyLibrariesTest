...
service_error_t service_client_factory_start_service(idevice_t device, const char* service_name, void **client, const char* label, int16_t (*constructor_func)(idevice_t, lockdownd_service_descriptor_t, void**), int16_t *error_code)
{
	*client = NULL;
	...
	if (LOCKDOWN_E_SUCCESS != lockdownd_client_new_with_handshake(device, &lckd, label)) {
		if (LOCKDOWN_E_SUCCESS != lockdownd_client_new_with_handshake(device, &lckd, label, true)) {
 ...
 */
service_error_t service_enable_ssl(service_client_t client, bool bTls)
{
	if (!client || !client->connection)
		return SERVICE_E_INVALID_ARG;

	idevice_error_t err = idevice_connection_enable_ssl(client->connection, bTls);
	//if (err == IDEVICE_E_SSL_ERROR)
	//{
	//	//retry use tls
	//	err = idevice_connection_enable_ssl(client->connection, true);
	//}
	
	return idevice_to_service_error(err);
}
