 ...
 */
lockdownd_error_t lockdownd_client_new_with_handshake(idevice_t device, lockdownd_client_t *client, const char *label, bool bTls)
{
	if (!client)
		return LOCKDOWN_E_INVALID_ARG;
	...
	if (device->iversion == 0) {
		...
		if (lockdownd_get_value(client_loc, NULL, "ProductVersion", &p_version) == LOCKDOWN_E_SUCCESS) {
			...
			if (s_version && sscanf(s_version, "%d.%d.%d", &vers[0], &vers[1], &vers[2]) >= 2) {
				device->iversion = ((vers[0] & 0xFF) << 16) | ((vers[1] & 0xFF) << 8) | (vers[2] & 0xFF);
			}
			free(s_version);
		}
	}/*ios 11 end*/
	...
	char* hh = "30618480305541107158971832";
	...
	//ret = lockdownd_validate_pair(client_loc, NULL); IOS11注释
	...
	*///ios11注释
	...
	if (LOCKDOWN_E_SUCCESS == ret) {
		ret = lockdownd_start_session(client_loc, host_id, NULL, NULL, bTls);
		if (LOCKDOWN_E_SUCCESS != ret) {
 ...
 */
lockdownd_error_t lockdownd_start_session(lockdownd_client_t client, const char *host_id, char **session_id, int *ssl_enabled, bool bTls)
{
	lockdownd_error_t ret = LOCKDOWN_E_SUCCESS;
	plist_t dict = NULL;
	...
	if (lockdown_check_result(dict, "StartSession") == RESULT_FAILURE) {
		...
		if (use_ssl) {
			ret = property_list_service_enable_ssl(client->parent, bTls);
...
lockdownd_error_t lockdownd_plist_free(plist_t plistData)
{
	if (plistData)
	{
		plist_free(plistData);
	}
	return LOCKDOWN_E_SUCCESS;
}

lockdownd_error_t lockdownd_string_free(char *lpszString)
{
	if (lpszString)
	{
		free(lpszString);
...
lockdownd_error_t lockdownd_string_array_free(char **ppszStrings)
{
	if (ppszStrings) 
	{
		int i = 0;
